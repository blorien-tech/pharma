...
@section('title', 'Dues Management - BLORIEN Pharma')
...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div class="bg-white rounded-lg shadow p-4">
            ...
            <p class="text-xs text-gray-500 mt-1">{{ $summary['count_pending'] }} pending dues</p>
        </div>
        <div class="bg-white rounded-lg shadow p-4">
            ...
            <p class="text-xs text-gray-500 mt-1">{{ $summary['count_overdue'] }} overdue</p>
        </div>
        <div class="bg-white rounded-lg shadow p-4">
            <h3 class="text-sm font-medium text-gray-600">Partial Payments</h3>
            <p class="text-2xl font-bold text-blue-600">{{ $summary['count_partial'] }}</p>
            <p class="text-xs text-gray-500 mt-1">Partially paid dues</p>
        </div>
        <div class="bg-white rounded-lg shadow p-4">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr class="{{ $due->isOverdue() ? 'bg-red-50' : '' }}">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                        ...
                        <div class="text-xs text-green-600">Paid: ৳{{ number_format($due->amount_paid, 2) }}</div>
                        @endif
                        <div class="text-xs font-medium text-yellow-700">Remaining: ৳{{ number_format($due->amount_remaining, 2) }}</div>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                        @if($due->isPaid())
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
                        <a href="{{ route('dues.show', $due) }}" class="text-blue-600 hover:text-blue-900">View</a>
                        @if(!$due->isPaid())
                        <a href="{{ route('dues.payment', $due) }}" class="text-green-600 hover:text-green-900">Collect</a>
                        @endif
                    </td>
                </tr>
                @empty
                <tr>
                    <td colspan="6" class="px-6 py-12 text-center">
                        <div class="text-gray-400">
                            ...
                            <p class="text-lg font-medium">No dues found</p>
                            <p class="text-sm mt-1">All dues have been collected or no dues recorded yet</p>
                        </div>
                    </td>
                </tr>
                @endforelse
