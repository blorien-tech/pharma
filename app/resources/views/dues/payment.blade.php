...
<div class="space-y-6">
    ...
    <div>
        ...
        <p class="mt-1 text-sm text-gray-600">Record payment for due #{{ $due->id }}</p>
    </div>
    ...
    <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-6">
        <div class="grid grid-cols-2 gap-4 mb-4">
            ...
            <div>
                <label class="block text-sm font-medium text-gray-600">Amount Remaining</label>
                <p class="text-2xl font-bold text-yellow-700">৳{{ number_format($due->amount_remaining, 2) }}</p>
            </div>
        </div>
        <div class="text-sm text-gray-600">
            <p>Total: ৳{{ number_format($due->amount, 2) }} | Paid: ৳{{ number_format($due->amount_paid, 2) }}</p>
        </div>
    </div>
        ...
        <form action="{{ route('dues.payment.store', $due) }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <select name="payment_method" id="payment_method" required
                    ...
                    <option value="MOBILE" {{ old('payment_method') == 'MOBILE' ? 'selected' : '' }}>Mobile Money (bKash/Nagad)</option>
            ...
            <div>
                ...
                <p class="mt-1 text-sm text-gray-500">Any additional information about this payment</p>
            </div>
    ...
    <div class="bg-white rounded-lg shadow p-6">
        <h3 class="text-sm font-medium text-gray-700 mb-3">Previous Payments ({{ $due->payments->count() }})</h3>
        <div class="space-y-2">
