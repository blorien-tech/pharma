...
<div class="space-y-6">
    <div class="flex justify-between items-center">
        <div>
            <h1 class="text-3xl font-bold text-gray-900">{{ __('reports.suppliers_title') }}</h1>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div class="bg-white rounded-lg shadow-md p-6">
            <div class="text-sm font-medium text-gray-500">{{ __('reports.total_purchases') }}</div>
            <div class="text-2xl font-bold text-gray-900 mt-2">৳{{ number_format($totalSpent, 2) }}</div>
        </div>
        <div class="bg-white rounded-lg shadow-md p-6">
            <div class="text-sm font-medium text-gray-500">{{ __('common.total') }}</div>
            <div class="text-2xl font-bold text-gray-900 mt-2">{{ $totalOrders }}</div>
        </div>
    </div>

    <div class="bg-white rounded-lg shadow-md p-6">
        <h2 class="text-lg font-semibold text-gray-900 mb-4">{{ __('reports.supplier_performance') }}</h2>
